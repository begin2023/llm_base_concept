...
再举个例子，对于 Attention，只要我们将 softmax 拿掉，即从 $\text{softmax}(Q K^{T}) V$变成 $QK^{T}V = Q(K^{T} V)$，将 $K^{T}$和 $V$先结合计算，总体的计算复杂度就从 $O(T^2 D)$变成了 $O(T D^2 )$，计算量不再随着序列长度呈二次方增长，这就是**线性注意力**。
